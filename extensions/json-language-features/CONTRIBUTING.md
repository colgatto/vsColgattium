...
- Open `/extensions/json-language-features/` as the workspace in VS Colgattium
...
- Run the [`Launch Extension`](https://github.com/microsoft/vscode/blob/master/extensions/json-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new VS Colgattium instance with the `json-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  VS Colgattium window opened on `json-language-features`.
